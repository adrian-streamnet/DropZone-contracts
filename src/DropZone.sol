...
contract DropZone is Ownable, Pausable {
    ...
    function updateMerkleRoot(
        bytes32 _merkleRoot,
        string calldata _merkleDataUri
    ) external onlyOwner {
        merkleRoot = _merkleRoot;
        merkleDataURI = _merkleDataUri;
        emit rootUpdated(_merkleRoot);
    ...
    function fundAirdrop(uint256 amount, address funder) external onlyOwner {
        require(
    ...
    /// @param _receiver The _receiver which receives the token
